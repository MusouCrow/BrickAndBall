...
namespace Game.Component.Brick_ {
	...
	public class Brick : LockBehaviour, IDrag {
		...
		protected override void LockUpdate() {
			...
			if (Judge.GameType == GameType.PVE && this.isPlayer) {
				this.Drag(Networkmgr.MousePosition, Input.GetMouseButton(0));
			}
		}
		...
		public void Drag(Vector3 position, bool isDown) {
			if (!isDown) {
				...
				this.OnDragEvent(this.draggingPos, position);
			}

			if (this.hasDraged) {
				this.draggingPos = position;
			}
		}
